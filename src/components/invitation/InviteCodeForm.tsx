...
export function InviteCodeForm({ variant = 'card' }: InviteCodeFormProps) {
  ...
  return (
    <div className={isHero ? 'invite-form-hero' : undefined}>
      <form onSubmit={(e) => void submit(e)} className="invite-form" style={{ display: 'flex', gap: 12, marginTop: isHero ? 0 : 28 }}>
        <input
          className="field"
          type="text"
          value={code}
          ...
          style={{
            ...
            flex: 1,
            minWidth: 0,
            ...(isHero ? { background: 'rgba(35,29,23,0.45)' } : {}),
          }}
        />
        <button
          type="submit"
          ...
          style={{
            ...
            borderRadius: 'var(--radius-house)',
            whiteSpace: 'nowrap',
            boxShadow: isHero ? 'var(--shadow-ember)' : '0 12px 40px rgba(160,74,42,0.45), inset 0 1px 0 rgba(255,255,255,0.18)',
          }}
        >
